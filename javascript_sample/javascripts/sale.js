...
if (isNaN(productId) || isNaN(number)) {
  window.alert("Por favor, seleccione un producto y la cantidad.");
  return;
}
...
purchases.forEach(purchase => {
  if (purchase.number === 1) {
    message += `${purchase.name} - ${purchase.price} yenes - Cantidad: ${purchase.number}`;
  }
  message += `\n${purchase.name} - ${purchase.price} yenes - Cantidad: ${purchase.number}`;
});
message += `\n`;
